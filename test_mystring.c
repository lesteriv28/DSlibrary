...
void testMyStrCpyWith(char *source) {
  ...
  free(result); // The memory allocated for the copied string is freed here
...
char *makeBigString() {
  char *bigString = malloc((BIG_STRING_LENGTH + 1) * sizeof(*bigString));
  memset(bigString, 'x', BIG_STRING_LENGTH + 1);
